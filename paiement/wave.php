...
// Configuration Wave API (à remplacer par les vraies clés)
$WAVE_API_URL = "https://api.wave.com/v1/checkout/sessions";
$WAVE_API_KEY = "VOTRE_API_KEY";  // À remplacer
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($telephone)) {
        $error = 'Veuillez entrer votre numéro Wave.';
    } else {
        // Simulation (à remplacer par API réelle)
        $paiement_reussi = true;
        
        if ($paiement_reussi) {
            $stmt = $pdo->prepare("UPDATE commandes SET statut = 'confirmee', mode_paiement = 'wave' WHERE id = ?");
            $stmt->execute([$commande_id]);
            
            $stmt = $pdo->prepare("
                INSERT INTO paiements (commande_id, mode, montant, statut, reference_transaction, created_at)
                VALUES (?, 'wave', ?, 'confirme', ?, NOW())
            ");
            $reference = 'WAVE-' . $commande['numero_commande'] . '-' . time();
            $stmt->execute([$commande_id, $commande['total'], $reference]);
...
<head>
    ...
    <style>
        ...
        .wave-icon { background: #1A7A4A; width: 70px; height: 70px; border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 20px; }
        .wave-icon i { font-size: 2rem; color: white; }
        .montant { font-size: 2rem; font-weight: bold; color: #C8922A; margin: 20px 0; }
        .btn-payer { background: #1A7A4A; color: white; border: none; padding: 14px; border-radius: 10px; font-weight: 600; width: 100%; cursor: pointer; }
        .btn-payer:hover { background: #145E38; }
...
<div class="paiement-container">
    <div class="paiement-card">
        ...
        <h2>Paiement Wave</h2>
        <p class="text-muted">Payez en toute sécurité avec Wave</p>
        ...
        <form method="POST">
            <div class="mb-3">
                ...
                <small class="text-muted">Entrez le numéro associé à votre compte Wave</small>
            </div>
