...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $pdo->prepare("
        UPDATE plats SET nom=?, description=?, prix=?, categorie_id=?, image=?, est_visible=?, est_plat_du_jour=?
        WHERE id=?
    ");
    $stmt->execute([$nom, $description, $prix, $categorie_id, $image, $est_visible, $est_plat_du_jour, $id]);
...
<head>
    ...
    <title>Modifier un plat - Admin</title>
        ...
        <form method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <?php if(!empty($plat['image'])): ?>
                    <img src="../uploads/plats/<?= $plat['image'] ?>" class="preview-img">
                <?php endif; ?>
            </div>
            <div class="row">
                <div class="col-md-6">
                    <div class="form-check mb-3">
                        <input type="checkbox" name="est_plat_du_jour" class="form-check-input" id="plat_jour" <?= $plat['est_plat_du_jour'] ? 'checked' : '' ?>>
                        <label class="form-check-label" for="plat_jour">⭐ Définir comme plat du jour</label>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="form-check mb-3">
                        <input type="checkbox" name="est_visible" class="form-check-input" id="visible" <?= $plat['est_visible'] ? 'checked' : '' ?>>
